...
function is_audio(ext) { return AUD_EXTS.indexOf(ext) >= 0; }

var ZIP_EXTS = 'zip rar gz tar 7z ace cab cfs dgc gca lha lzh boz bz bz2';
function is_compressed(ext) { return ZIP_EXTS.indexOf(ext) >= 0; }
...
function is_image(ext) { return IMG_EXTS.indexOf(ext) >= 0; }

function is_pdf(ext) { return ext === 'pdf'; }

var PPT_EXTS = (
    'gslides odp otp pot potm potx ppam pps ppsm ppsx ppt pptm pptx '
    + 'sldm sldx'
);
function is_presentation(ext) { return PPT_EXTS.indexOf(ext) >= 0; }

var XLS_EXTS = 'csv gsheet ods ots xla xlam xlc xlm xls xlsb xlsm xlsx xlt xltm xltx xlw xml';
function is_spreadsheet(ext) { return XLS_EXTS.indexOf(ext) >= 0; }

var TXT_EXTS = 'conf def in ini list log text txt';
function is_text(ext) { return TXT_EXTS.indexOf(ext) >= 0; }
...
function is_video(ext) { return VID_EXTS.indexOf(ext) >= 0; }

var WRD_EXTS = (
    'abw clkw doc docm docx dot dotm dotx gdoc '
    + 'kwd kwt lwp odt ott wpd'
);
function is_word(ext) { return WRD_EXTS.indexOf(ext) >= 0; }
...
var Filetype = {
    get_file_type: function(ext) { return ALL_EXTS[ext]; },
    get_filename: function(path) {
        return ((path.split('#')[0]).split('?')[0]).split('/').pop();
    },
    get_file_ext: function(path) {
        return this.get_filename(path).split('.').pop();
    },
    set_file_info: function(obj) {
        obj.class = get_icon_class(obj.extension);
        obj.can_preview = [0, 2, 6].indexOf(_CLASSES.indexOf(obj.class));
        return obj;
    },
    to_images_list: function(v) {
        var ret = [];
        for (var i = 0, l = v.length, d; i < l; i++) {
            d = v[i];
            if (d == null || $type(d) !== 'String' || !d.length) continue;
            d = d.toLowerCase();
            if (ret.indexOf(d) < 0 && this.is_ext_image(v)) ret.push(d);
        }
        return ret;
    },
    is_ext_image: function(v) {
