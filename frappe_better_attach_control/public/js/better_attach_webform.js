  ...
  frappe.ui.form.ControlAttach = class ControlAttach extends frappe.ui.form.ControlAttach {
    ...
    _set_max_attachments() {
      if (!this.frm)
        return;
      if (!this._allow_multiple) {
        let meta2 = frappe.get_meta(this.frm.doctype);
        if (meta2)
          meta2.max_attachments = this._max_attachments[0];
        if (this.frm.meta)
          this.frm.meta.max_attachments = this._max_attachments[1];
        return;
      }
      let val = this._options.restrictions.max_number_of_files, meta = frappe.get_meta(this.frm.doctype);
